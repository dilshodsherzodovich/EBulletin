...
export const baseSchemas = {
  ...
  phoneNumber: z.string().regex(/^\+?[1-9]\d{1,14}$/, validationMessages.phoneNumber),
  url: z.string().url(validationMessages.url),
  requiredString: (minLength = 1) =>
    z.string().min(minLength, minLength === 1 ? validationMessages.required : validationMessages.minLength(minLength)),
  optionalString: (maxLength?: number) =>
    z
      .string()
      .optional()
      .refine((val) => !maxLength || !val || val.length <= maxLength, {
        message: maxLength ? validationMessages.maxLength(maxLength) : undefined,
      }),
  dateString: z.string().refine((val) => !val || !isNaN(Date.parse(val)), { message: validationMessages.invalidDate }),
  futureDate: z
    .string()
    .refine((val) => !val || new Date(val) > new Date(), { message: validationMessages.futureDate }),
  pastDate: z.string().refine((val) => !val || new Date(val) < new Date(), { message: validationMessages.pastDate }),
  numericString: z.string().regex(/^\d+$/, validationMessages.numeric),
  alphanumericString: z.string().regex(/^[a-zA-Z0-9]+$/, validationMessages.alphanumeric),
}

// Document-specific validation schemas
export const documentSchemas = {
  documentTitle: baseSchemas.requiredString(3).max(200, validationMessages.maxLength(200)),
  ...
  documentVersion: z.string().regex(/^\d+\.\d+(\.\d+)?$/, "Формат версии: X.Y или X.Y.Z"),
  documentAuthor: baseSchemas.requiredString(2).max(100, validationMessages.maxLength(100)),
...
export const userSchemas = {
  firstName: baseSchemas.requiredString(2).max(50, validationMessages.maxLength(50)),
  lastName: baseSchemas.requiredString(2).max(50, validationMessages.maxLength(50)),
  email: baseSchemas.email,
  phoneNumber: baseSchemas.phoneNumber.optional(),
  position: baseSchemas.requiredString(2).max(100, validationMessages.maxLength(100)),
...
export const createFileValidation = (
  ...
  return z.object({
    file: z
      .instanceof(File)
      .refine((file) => file.size <= maxSizeMB * 1024 * 1024, validationMessages.fileSize(maxSizeMB))
      .refine((file) => {
...
export const formSchemas = {
  ...
  contactForm: z.object({
    name: baseSchemas.requiredString(2).max(100, validationMessages.maxLength(100)),
    email: baseSchemas.email,
    subject: baseSchemas.requiredString(5).max(200, validationMessages.maxLength(200)),
    message: baseSchemas.requiredString(10).max(2000, validationMessages.maxLength(2000)),
  }),
}
